...
//             ws.onclose = (e) => {
//                 console.log("🔴 Global Chat Disconnected. Attempting auto-reconnect in 3 seconds...", e.reason);
...
//                 wsRef.current.send(JSON.stringify({ type: "ping", message: "" }));
//             }
//         }, 25000);
...
const GlobalChat = ({ currentUserRole, currentUserName, communityId }) => {
    ...
    // 🎯 FIX: Fetch history ONCE when communityId is available, not when isOpen changes!
    useEffect(() => {
        if (!communityId) return;
        ...
        const fetchHistory = async () => {
            ...
            try {
                const res = await axiosInstance.get('/chat/global/history/'); 
                const history = Array.isArray(res.data) ? res.data : res.data.results || [];
                
                // Merge initial history with any live messages already received
                setMessages((prev) => {
    ...
    useEffect(() => {
        ...
        const connectWebSocket = () => {
            ...
            ws.onmessage = (event) => {
                ...
                // Append incoming live message seamlessly
                setMessages((prev) => [...prev, data]);
            };

            ws.onclose = (e) => {
                console.log("🔴 Global Chat Disconnected. Reconnecting in 3s...", e.reason);
                reconnectTimeout = setTimeout(() => {
                    connectWebSocket();
                }, 3000);
            };
