    ...
    useEffect(() => {
        ...
        syncSocket.onmessage = (event) => {
            try {
                        ...
                        prev.map((item) =>
                            item.id === data.issue.id
                                ? { 
                                    ...
                                    assigned_staff_name: data.issue.assigned_staff_name 
                                  }
                                : item
                        )
                    );
                ...
                console.error("Resident socket packet parse error:", err);
            }
        };
    ...
    const fetchIssues = async () => {
        ...
        try {
            ...
            setPopup({
                isOpen: true,
                status: 'error',
                message: err.response?.data?.error || err.response?.data?.detail || 'Failed to load your issues.'
            });
        } finally {
            setLoading(false);
        }
            ...
            const fetchChatHistory = async () => {
                try {
                    ...
                    setPopup({
                        isOpen: true,
                        status: 'error',
                        message: err.response?.data?.error || err.response?.data?.detail || 'Failed to load chat history.'
                    });
                }
            };
            fetchChatHistory();
                                        ...
                                        {selectedIssue.assigned_staff_name ? (
                                            <>
                                                <div className="flex items-center gap-3 mb-3">
                                                    ...
                                                    <p className="text-lg font-bold text-slate-200">{selectedIssue.assigned_staff_name} {selectedIssue.assigned_staff_designation ? `- ${selectedIssue.assigned_staff_designation}` : ''}</p>
                                                </div>
                                                <div className="grid grid-cols-1 gap-2 text-sm sm:grid-cols-2 text-slate-400">
                                                    <p><span className="font-semibold text-slate-500">Phone:</span> <span className="text-slate-300">{selectedIssue.assigned_staff_phone || 'N/A'}</span></p>
                                                    <p><span className="font-semibold text-slate-500">Email:</span> <span className="text-slate-300">{selectedIssue.assigned_staff_email || 'N/A'}</span></p>
                                    ...
                                    {selectedIssue.uploaded_images && selectedIssue.uploaded_images.length > 0 && (
                                        <>
                                            ...
                                            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
                                                {selectedIssue.uploaded_images.map((img, i) => (
                                                    <a key={i} href={img.image} target="_blank" rel="noopener noreferrer">
                                                        <img src={img.image} alt="Issue attachment" className="block object-cover w-full h-32 transition-transform border rounded-lg shadow-md border-slate-700 hover:scale-105" />
                                                    </a>
                                                ))}
                                            </div>
                                        </>
                                    ...
                                    <div className="flex-1 p-4 overflow-y-auto space-y-4 bg-slate-950/50 min-h-[300px]">
                                        {chatMessages.length === 0 ? (
                                            ...
                                            chatMessages.map((msg, index) => {
                                                ...
                                                const isAdmin = msg.sender_role === 'ADMIN';
                                                const isResident = msg.sender_role === 'RESIDENT';
                                                const isMe = msg.sender_name === user?.name || isResident;
                                                ...
                                                return (
                                                    <div key={index} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                                                        ...
                                                        <div className={`px-4 py-2.5 rounded-2xl max-w-[80%] text-sm shadow-md ${
                                                            isMe ? 'bg-rose-600 text-white rounded-tr-sm' :
                                                            isStaff ? 'bg-cyan-700 text-white rounded-tl-sm' : 
                                                            'bg-slate-800 text-slate-200 rounded-tl-sm border border-slate-600'
                                                        }`}>
                                                            {msg.message}
                                                        </div>
                                                        <span className="text-[10px] text-slate-500 mt-1">{msg.timestamp || 'Synced'}</span>
                                                    </div>
                                                );
                                            })
                                        )}
                                    ...
                                    <form onSubmit={sendMessage} className="p-4 border-t border-slate-800 bg-slate-900">
                                        <div className="flex gap-2">
                                            <input 
                                                ...
                                                value={newMessage}
                                                onChange={(e) => setNewMessage(e.target.value)}
                                                placeholder={`Message ${selectedIssue.assigned_staff_name || 'support'}...`}
                                                className="flex-1 px-4 py-3 text-sm border outline-none bg-slate-800 text-slate-200 border-slate-700 rounded-xl focus:border-cyan-500"
                                            />
                                            <button 
                                                type="submit"
