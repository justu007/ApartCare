...
#                 sender = getattr(self.user,'name', 'A Resident')
...
#                             "message": f"one message received by '{sender}' - {message[:20]}..."
#                         }
#                     )
...
#         receivers = User.objects.filter(community_id = self.room_id).exclude(id = self.user.id)
#         sender = getattr(self.user,'name', 'A Resident')
