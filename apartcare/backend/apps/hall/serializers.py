...
class HallBookingSerializer(serializers.ModelSerializer):
    ...
    def get_flat_name(self, obj):
        try:
            return obj.resident.resident_profile.flat.name if obj.resident.resident_profile.flat else "N/A"
        except Exception:
            return "N/A"
    ...
    def validate(self, data):
        ...
        if not self.instance:
            ...
            if overlapping.exists():
                raise serializers.ValidationError({"time_slot": f"This time slot is already booked for {hall.name}."})
